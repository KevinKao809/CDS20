...
class EditInputInfo {
  ...
  description: String;
  id: String;
  constructor(name: String = null, dataType: String = null, description: String = null,mandatoryFlag: Boolean=null,id:String=null) {
    this.name = name;
...
@Component({
  selector: 'app-mandatory-message',
  templateUrl: './mandatory-message.component.html',
  styleUrls: ['./mandatory-message.component.scss']
})

export class MandatoryMessageComponent implements OnInit {
  // Table layout config
  tableConfig = {
    'column': ['No', 'Name', 'Data Type', 'Mandatory', 'Description'],
    'columnClass': [5, 25, 20, 20, 30],
    'data': [],
    'canExpand': true,
    'expandTrigger': 'row'
  };
  ...
  selectRowUpdate(selectRowInfo) {
    ...
    this.cancelNewPanel();
    this.apiService.getMandatoryMessage(parseInt(selectRowInfo.Id, 10)).subscribe(
      result => {
        this.detailInfo = new EditInputInfo(result.ElementName,
                                            result.ElementDataType,
                                            result.Description,
                                            result.MandatoryFlag,
                                            result.Id);
   ...
   */
  deleteItem() {
    this.apiService.deleteMandatoryMessage(this.currentRow.Id).subscribe(
      result => {
        ...
        this.sharedService.publishPopupWindowMsg({
          'type': 'feedback',
          'id': 'deleteMandatoryMessageSuccess'
        });
      },
      error => {
          console.error(error);
  ...
  confirmEdit(editForm: NgForm) {
    if (editForm.valid) {
      ...
      const updateObj = {
        'ElementName': this.detailInfo.name,
        'Description': this.detailInfo.description,
        'ElementDataType': this.detailInfo.dataType,
        'MandatoryFlag': true
      };
      
      // Create new Item
      if (this.toTableCommamd.isNew) {
        this.apiService.addMandatoryMessage(updateObj).subscribe(
          result => {
            this.tableConfig.data[0]['No'] = this.tableConfig.data.length;
            this.tableConfig.data[0]['Name'] = updateObj.ElementName;
            this.tableConfig.data[0]['Data Type'] = updateObj.ElementDataType;
            this.tableConfig.data[0]['Description'] = updateObj.Description;
            this.tableConfig.data[0]['Default Value'] = updateObj.MandatoryFlag;
            ...
            this.sharedService.publishPopupWindowMsg({
              'type': 'feedback',
              'id': 'createMandatoryMessageSuccess'
            });
          },
          error => {
              console.error(error);
      ...
      // Edit specific Item
        ...
        this.apiService.editMandatoryMessage(this.currentRow.Id, updateObj).subscribe(
          result => {
            for (let i = 0; i < this.tableConfig.data.length; i++) {
              if (this.tableConfig.data[i].Id === this.currentRow.Id) {
                this.tableConfig.data[i]['Name'] = updateObj.ElementName;
                this.tableConfig.data[i]['Data Type'] = updateObj.ElementDataType;
                this.tableConfig.data[i]['Description'] = updateObj.Description;
                this.tableConfig.data[i]['Default Value'] = updateObj.MandatoryFlag;
                this.updateTableContent(this.tableConfig);
              }
            }
            this.sharedService.publishPopupWindowMsg({
              'type': 'feedback',
              'id': 'editMandatoryMessageSuccess'
            });
          },
          error => {
              console.error(error);
  ...
  ngOnInit() {
    // get SuperAdmin Info
    this.apiService.getMandatoryMessageList().subscribe(
      result => {
        for (let i = 0; i < result.length; i++) {
          this.tableConfig.data.push({
            'Id': result[i].Id,
            'No': (i + 1),
            'Name': result[i].ElementName,
            'Data Type': result[i].ElementDataType,
            'Description': result[i].Description,
            'Mandatory': result[i].MandatoryFlag
          });
        }
        this.updateTableContent(this.tableConfig);
      },
