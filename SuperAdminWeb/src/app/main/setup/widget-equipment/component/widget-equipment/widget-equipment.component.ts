  ...
  this.apiService.deleteWidgetKeyInfo(this.currentRow['Widget Key']).subscribe(
    result => {
      ...
      this.sharedService.publishPopupWindowMsg({
        'type': 'feedback',
        'id': 'deleteWidgetEquipmentSuccess'
      });
    },
    error => {
        console.error(error);
    ...
    if (this.toTableCommamd.isNew) {
      ...
      this.apiService.addWidgetInfo(updateObj).subscribe(
        result => {
          ...
          this.sharedService.publishPopupWindowMsg({
            'type': 'feedback',
            'id': 'createWidgetEquipmentSuccess'
          });
        },
        error => {
            console.error(error);
...
ngOnInit() {
  // get List Info
  this.apiService.getWigetClassList("equipment").subscribe(
    result => {
