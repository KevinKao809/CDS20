...
namespace EmailApp.Model
{
    public class Heartbeat
    {
        ...
        public static void Start()
        {
            ...
            EmailApp._appLogger.Info("Heartbeat Interval: " + _heartbeatIntervalinSec + " seconds");
        }
        ...
        private static void PushHeartbeatSignal(object state)
        {
            ...
            catch (Exception ex)
            {
                ...
                logMessage.AppendLine("Email Task Exception on send Heartbeat: " + ex.Message);
        ...
        private static string GetHeartbeatStatus()
        {
            ...
            HeartbeatMessage.name = "Email Task";
            HeartbeatMessage.machine = Environment.MachineName;
            HeartbeatMessage.processId = _ProcessId;
            HeartbeatMessage.incomeMessage = EmailApp._incomeMessage;
            HeartbeatMessage.processedMessage = EmailApp._processedMessage;
            HeartbeatMessage.failMessage = EmailApp._failMessage;
            HeartbeatMessage.ignoreMessage = EmailApp._ignoreMessage;

            if (EmailApp._isRunning)
                HeartbeatMessage.status = "Running";
            else
                HeartbeatMessage.status = "Stop";

            if (_sbNameSpaceMgr != null)
                HeartbeatMessage.queueLength = _sbNameSpaceMgr.GetQueue(EmailApp._sbEmailQueue).MessageCount;
            else
