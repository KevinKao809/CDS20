...
{
    [AllowAnonymous]
    [RoutePrefix("AdminHeartbeat")]
    public class AdminHeartbeatController : Controller
    {
        [HttpPost]
        [Route("FeedIn")]
        // Post: AdminHeartbeat
        public ActionResult FeedIn()
        {
            ...
            using (StreamReader reader = new StreamReader(memstream))
            {
                ...
                Startup._appLogger.Info(string.Format("AdminHeartbeat Received: {0}", feedInData));
            }
            try
            {
            ...
            {
                Startup._appLogger.Error(string.Format("AdminHeartbeat Exception: {0}", ex.Message));
                return this.Content("{\"Result\":\"Fail\"}", "application/json");
            }
        }

        /* The Heartbeat message come to Admin Web is: */
        /* - IoTHub Receiver   */
